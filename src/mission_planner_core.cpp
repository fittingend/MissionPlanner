...
/**
 * @brief start/goal + 중간 waypoint 들을 이용해 DefaultPlannerCore 로부터 Route 생성
 *
 * 인자로 들어오는 start_pose / waypoints/ goal_pose 는
 * 모두 header.frame_id 좌표계라고 가정.
 * 내부에서 transform_pose_ 를 통해 map_frame_ 으로 변환한 뒤 DefaultPlannerCore::plan() 에 전달.
 */
  ...
  const bool allow_goal_modification) const
{
  ...
  //     이제는 DefaultPlannerCore::RoutePoints = std::vector<Pose> 그대로 사용
  DefaultPlannerCore::RoutePoints points;
  points.clear();
